...
**等待上传完成**：轮询 textarea 周围是否有 spinner 文本（"上传中"等），最多 5 分钟。
...
### `frontend/src/views/ImagePublish.vue` — 4 处改动

1. `IMAGE_PLATFORM_KEYS` 加 `'weibo'`
2. `import WeiboImagePublishPanel` + `const weiboPanelRef = ref(null)`
3. `getPanel` map / `hasAccountOverride` 循环 / `saveDraft` 循环 / `migrateOldDraftFormat` 循环 — 4 处 hardcoded 平台数组加 `'weibo'`
4. 模板加 `<WeiboImagePublishPanel v-show="selectedPlatform === 'weibo'" ...>`
...
1. 启动 dev server（`backend/python3 app.py` + `frontend/npm run dev`）
2. 登录一个微博账号
...
- **回滚**：5 个核心文件 + 1 个新文件 + 1 个新 panel，git revert 即可，原子性强。
