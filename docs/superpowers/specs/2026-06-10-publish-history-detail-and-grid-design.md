...
1. **账号信息头**（`account-header`，数据源：`selectedItem`）
   - 头像（取 `account.name` 首字母，沿用 `AccountSidebar` 的样式）+ 平台徽章 + 账号名 + 状态标签 + 错误信息（仅 `selectedItem.status === 'failed'` 时显示）+ 「查看发布作品」链接（仅 `selectedItem.status === 'success'` 且 `selectedItem.publish_url` 非空时渲染，`target="_blank" rel="noopener noreferrer"`）
...
| `accounts` | `Account[]` | `[]` | 取代从 store 取（实际视图仍由 `accountGroups` 计算，但 readonly 下用此 prop 入参） |

`Account` 形状沿用 `useAccountStore` 已有的：`{id, name, platform, filePath, status, ...}`（详见 `frontend/src/stores/account.js`）。readonly 场景下，调用方传入过滤后的子集。
