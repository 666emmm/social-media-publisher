...
class XPlatform(BasePlatform):
    ...
    async def _publish(self, **kwargs):
        ...
        # Build full text (title optional for X)
        text_parts = []
        if title:
    ...
    async def _fill_text(self, page, text: str):
        ...
        try:
            ...
            # 跳过 click()：上传视频后编辑器可能在视口外，bounding_box 会超时
            # JS evaluate 中的 el.focus() 足以完成聚焦
            try:
                await text_area.click(timeout=2000)
                await asyncio.sleep(0.3)
            except Exception:
                logger.info("[填写] click 跳过（元素不在视口），直接用 JS 注入")
                await asyncio.sleep(0.3)
