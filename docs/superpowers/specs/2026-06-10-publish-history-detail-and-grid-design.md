...
4. 详情页右侧（按 `PublishCenter` 范式，账号栏在左、主区在右）复用 `AccountSidebar` readonly 模式展示该批次的所有账号
5. 详情页主区域选中账号后展示：账号头 + 内容快照 + 数据占位区
...
| 3 | 渠道徽章行 | 复用 `ChannelSummary.vue` 组件，溢出时 marquee（沿用 `DraftBox.vue` 动画） |
...
Props：`channels: Array<{platform, name, count, logo}>`、`draftId: String|Number`（用于 marquee 检测 key，可选）。
...
**左侧栏（`detail-sidebar`，232px 宽）：**
- `<AccountSidebar mode='readonly' :accounts="..." :account-groups="..." ...>`
- 顶部徽章标题改"批次账号 (N)"
- 平台分组自动展开所有有账号的组（mount 时 `expandedGroups = new Set(accountGroups.map(g => g.key))`）

**主区域（`detail-main`，`flex: 1`，`overflow-y: auto`）：**
纵向 4 区块，24px 间距：

1. **账号信息头**（`account-header`）
   - 头像（account.avatar 或首字母）+ 平台徽章 + 账号名 + 状态标签 + 错误信息（如有）+ 「查看发布作品」链接（`item.publish_url`，新窗口）
2. **内容快照**（`content-snapshot`）
   - 左封面（16:9，160px 宽） + 右文字（标题/描述 2 行省略/标签 el-tag/作品声明/定时发布时间/发布耗时）
   - 失败时降级为错误展示
3. **数据统计**（`data-stats`）
   - `<PublishStats />`，值 `--`
4. **批次元信息**（`batch-meta`，`<el-collapse>` 折叠，默认收）
   - 批次 ID、批次 schedule_time、started_at、finished_at、account_count

**选中默认行为：** 详情页 mount 时若 `selectedAccountId` 为空：
1. 从 `batch.items[0].account_id` 取（首选）
2. 若 `items[0].account_id` 为 null/缺失（账号已删除场景），回退到 `items[0].id`（明细行 UUID）
3. 若 `items` 本身为空 → 走空状态分支

**数据派生：**
- `batchAccounts = computed(() => accountStore.accounts.filter(a => batch.value?.items.some(it => it.account_id === a.id)))`
- `readonlyAccountGroups = computed(() => platformList.map(p => ({ key: p.key, name: p.name, logo: p.logo, color: p.color, letter: p.letter, accounts: batchAccounts.value.filter(a => a.platform === p.name) })))`
- `selectedItem = computed(() => batch.value?.items.find(it => it.account_id === selectedAccountId.value || it.id === selectedAccountId.value))`

**空状态：**
- 后端 404 → 跳回 `PublishHistory` + toast "记录不存在或已被删除"
- `batch.items` 为空 → 主区域 "该批次暂无账号数据"
...
| `accounts` | `Account[]` | `[]` | 取代从 store 取（实际视图仍由 `accountGroups` 计算，但分组在 readonly 下用此 prop 入参） |
...
- 从 `publish_batches` 查 1 行；找不到 → `404 {code: 404, msg: "记录不存在或已被删除"}`
- 找到 → 从 `publish_details WHERE batch_id = ?` 查所有明细
- 明细行 `account_configs` JSON 反序列化、`duration` 计算（沿用列表端点逻辑）
- 返回结构与列表 item 一致：`{id, type, title, ..., cover_url, account_count, success_count, failed_count, status, ..., items: [...]}`
...
| 单个 item 失败 | 账号头降级：红标 + 错误信息替代内容快照 | `item.status === 'failed'` |
| 账号已被删除 | 侧边栏该项 "已删除账号"占位，禁用点击 | `account_id` 在 store 查不到 |
| URL 不带 batchId | 路由守卫跳回列表 | 路由层 |
...
`frontend/src/components/__tests__/`（或同等位置）：
